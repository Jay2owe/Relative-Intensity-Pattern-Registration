...
class LogPlane:
    ...
    def _blur5(values: np.ndarray, valid: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        ok = valid.astype(np.float32)
        weighted = np.where(valid, values, 0).astype(np.float32)
        horizontal_sum = ndimage.convolve1d(weighted, kernel, axis=1, mode="nearest")
        horizontal_weight = ndimage.convolve1d(ok, kernel, axis=1, mode="nearest")
        horizontal = np.divide(
            horizontal_sum,
            horizontal_weight,
            out=np.zeros_like(horizontal_sum),
            where=horizontal_weight > 0,
        ).astype(np.float32)
        share = (horizontal_weight / 16.0).astype(np.float32)
        vertical_sum = ndimage.convolve1d(horizontal * share, kernel, axis=0, mode="nearest")
        vertical_weight = ndimage.convolve1d(share, kernel, axis=0, mode="nearest")
        output = np.divide(
            vertical_sum,
            vertical_weight,
            out=np.zeros_like(vertical_sum),
            where=vertical_weight > 0,
        ).astype(np.float32)
        return output, vertical_weight / 16.0 >= 0.5
    ...
    def sample(self, x: np.ndarray, y: np.ndarray, field: str = "value") -> tuple[np.ndarray, np.ndarray]:
        ...
        return (top + fy * (bottom - top)).astype(np.float64), ok
...
def align_area(a: Sequence[LogPlane], b: Sequence[LogPlane], options: AlignerOptions, estimator: Estimator) -> PairFit:
    if options.fit_rotation:
        return _align_area_rigid(a, b, options, estimator)
    top = len(a) - 1
    transform, found = IDENTITY, False
    for level in range(top, -1, -1):
        scale = 1 << level
        max_here = options.max_shift / scale
        wa, wb = _AreaWindow.of(a[level], options), _AreaWindow.of(b[level], options)
        radius = math.ceil(max_here) if level == top else 2
...
def _align_area_rigid(
    a: Sequence[LogPlane], b: Sequence[LogPlane], options: AlignerOptions, estimator: Estimator
) -> PairFit:
...
def _estimate_pair(a, b, options: RegistrationOptions, start=None, support=None):
    if start is not None or support is not None:
        return align_log_ratio(a, b, options.aligner, start, support)
    if options.estimator is Estimator.LOG_RATIO_FIT:
        return align_log_ratio(a, b, options.aligner)
    return align_area(a, b, options.aligner, options.estimator)
