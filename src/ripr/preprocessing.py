...
def apply_preprocessing(image: np.ndarray, choice: Preprocessing | str) -> np.ndarray:
    ...
    if choice is Preprocessing.LOCAL_CONTRAST_1_8:
        radius = max(2, int(np.floor(min(source.shape) / 16.0 + 0.5)))
        size = 2 * radius + 1
        finite = np.isfinite(source)
        values = np.where(finite, source, 0.0).astype(np.float64)
        counts = finite.astype(np.float64)
        total = ndimage.uniform_filter(values, size=size, mode="constant", cval=0.0)
        count = ndimage.uniform_filter(counts, size=size, mode="constant", cval=0.0)
        background = np.divide(total, count, out=np.full_like(total, np.nan), where=count > 0)
        output = np.where(finite & np.isfinite(background), source - background, np.nan)
        valid = output[np.isfinite(output)]
        if valid.size and np.min(valid) < 0:
            output[np.isfinite(output)] -= np.min(valid)
