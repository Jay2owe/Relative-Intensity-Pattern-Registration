...
public class RelativeIntensityPatternSweepResidualTest {
    @Test
    public void correctedFullResolutionResidualBeatsIdentityOnShiftedStack() {
        ...
        RelativeIntensityPatternParameters parameters = RelativeIntensityPatternParameters.builder().useRecommendation(false)
                .reference(Reconciler.Reference.CONSECUTIVE).autoMaxShift(false).maxShift(10)
                .threads(1).crop(false).build();
        Registration.Result registered = RelativeIntensityPatternRegistration.estimate(image, parameters,
        ...
        Registration.Result identity = RelativeIntensityPatternRegistration.estimate(staticImage, parameters,
                PairScheduler.Progress.NONE, PairScheduler.Cancellation.NEVER);
        ...
        double corrected = RelativeIntensityPatternSweepDialog.fullResolutionResidual(image, parameters, registered);
        double uncorrected = RelativeIntensityPatternSweepDialog.fullResolutionResidual(image, parameters, identity);
