...
public class RobustNormTest {
    ...
    /**
     * The scale, computed the slow obvious way: absolute values, sorted, indexed. Deliberately
     * naive, because it is the thing the fast path is asserted against.
     */
    private static double scaleBySorting(double[] r, int n, double floor) {
        if (n <= 0) return floor;
        double[] absolute = new double[n];
        for (int i = 0; i < n; i++) absolute[i] = Math.abs(r[i]);
    ...
    /**
     * The scale skips the selection entirely when the zeros it counted already outnumber half the
     * field, and drops them before selecting when the high quantile is needed. Both are shortcuts
     * through a quantile of a non-negative array, so neither may move the answer by anything at
     * all — exercised across the whole range of zero fractions, because the two branches are
     * chosen by that fraction and a test that only sees one of them proves nothing.
     */
    @Test
    public void scaleMatchesASortedReferenceAtEveryZeroFraction() {
        Random rng = new Random(20260910);
        for (int trial = 0; trial < 400; trial++) {
            int n = 1 + rng.nextInt(300);
            double zeroFraction = rng.nextInt(11) / 10.0;
            double[] r = new double[n];
            for (int i = 0; i < n; i++) {
                r[i] = rng.nextDouble() < zeroFraction
                        ? (rng.nextBoolean() ? 0.0 : -0.0)
                        : rng.nextGaussian() * 0.03;
            }
            double floor = rng.nextBoolean() ? 1e-9 : 1e-3;
            assertEquals("n=" + n + " zeroFraction=" + zeroFraction,
                    scaleBySorting(r.clone(), n, floor),
                    RobustNorm.scale(r.clone(), n, floor), 0);
        }
    }

    /**
     * A mostly-empty field with a bright minority: the fallback's reason to exist, and the only
     * route into the branch that drops the zeros before selecting. 15% bright is chosen so the
     * median is still zero while the 90th percentile is not — at 4% both land in the zero run and
     * the compaction is never reached, which is a test that proves nothing.
     */
    @Test
    public void scaleEscalatesToTheHighQuantileWithoutMovingIt() {
        int n = 1000;
        double[] r = new double[n];
        for (int i = 0; i < 150; i++) r[n - 1 - i] = 0.5 + i;
        assertEquals(scaleBySorting(r.clone(), n, 1e-9), RobustNorm.scale(r.clone(), n, 1e-9), 0);
        assertTrue("the median is zero, so the high quantile is what must be reported",
                RobustNorm.scale(r.clone(), n, 1e-9) > 1e-9);
    }

    /** Exactly at the boundary, where an off-by-one in the counted run would show. */
    @Test
    public void scaleIsExactWhereTheZeroRunEndsAtTheMedian() {
        for (int n = 2; n <= 40; n++) {
            for (int zeros = 0; zeros <= n; zeros++) {
                double[] r = new double[n];
                for (int i = zeros; i < n; i++) r[i] = 0.25 * (i - zeros + 1);
                // shuffled, so the count cannot accidentally agree by the zeros being in front
                Random rng = new Random(n * 100L + zeros);
                for (int i = n - 1; i > 0; i--) {
                    int j = rng.nextInt(i + 1);
                    double t = r[i]; r[i] = r[j]; r[j] = t;
                }
                assertEquals("n=" + n + " zeros=" + zeros,
                        scaleBySorting(r.clone(), n, 1e-9), RobustNorm.scale(r.clone(), n, 1e-9), 0);
            }
        }
    }
}
