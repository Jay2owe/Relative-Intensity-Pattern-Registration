...
public class PairSchedulerTest {
    ...
    /** Expensive first pairs must become visibly active before one of them finishes. */
    @Test
    public void lifecycleEventsArriveBeforeTheFirstCompletion() {
        ...
        PairScheduler.Progress progress = new PairScheduler.Progress() {
            ...
            @Override public void update(int done, int total) {
                // Completion is deliberately not the event under test.
            }
        };
        PairScheduler.map(1, 1, index -> {
            assertTrue("batch start must be visible before work", began.get());
            assertTrue("active pair must be visible before work", started.get());
            return index;
        }, progress, PairScheduler.Cancellation.NEVER);
    }
