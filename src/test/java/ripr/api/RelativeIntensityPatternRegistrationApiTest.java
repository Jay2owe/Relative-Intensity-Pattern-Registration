...
public class RelativeIntensityPatternRegistrationApiTest {
    @Test
    public void longitudinalModeIsASeparateWholeRecordingRoute() {
        int width = 48;
        ImageStack stack = new ImageStack(width, width);
        for (int frame = 0; frame < 6; frame++) {
            float[] pixels = new float[width * width];
            int shift = frame >= 4 ? 4 : 0;
            double gain = frame == 1 ? 1.8 : frame == 2 ? 0.6 : 1.0;
            for (int y = 0; y < width; y++) for (int x = 0; x < width; x++) {
                int sourceX = x - shift;
                double blob = sourceX < 0 ? 0 : 120 * Math.exp(
                        -((sourceX - 17.0) * (sourceX - 17.0)
                        + (y - 25.0) * (y - 25.0)) / 55.0);
                pixels[y * width + x] = (float) (gain * (8 + blob));
            }
            stack.addSlice(new FloatProcessor(width, width, pixels));
        }
        ImagePlus input = new ImagePlus("longitudinal", stack);
        RelativeIntensityPatternParameters parameters =
                RelativeIntensityPatternParameters.builder()
                        ...
                        .autoMaxShift(false).maxShift(10).threads(1).crop(false).build();

        RelativeIntensityPatternResult result =
                RelativeIntensityPatternRegistration.register(input, parameters);
        try {
            assertEquals(SelectionMode.LONGITUDINAL_ACCURACY,
                    result.parameters().selectionMode);
            assertNotNull(result.longitudinalDiagnostics());
            assertEquals("bright_dim_references",
                    result.longitudinalDiagnostics().route);
            assertTrue(result.provenance().contains("selected_channel_only=true"));
            assertEquals(6, result.registration().cumulative.length);
        } finally {
            result.close();
            input.close();
        }
    ...
    @Test
    public void publicRigidSettingsMapDegreesToCoreRadians() {
        RelativeIntensityPatternParameters defaults = RelativeIntensityPatternParameters.builder().build();
        assertFalse(defaults.fitRotation);
        assertFalse(defaults.registrationOptions().aligner.fitRotation);
        assertTrue(defaults.incrementalRotation);
        assertTrue(defaults.registrationOptions().incrementalRotation);
        assertEquals(0.005, defaults.minimumRotationResidualGain, 0.0);
        assertEquals(0.005, defaults.registrationOptions().minimumRotationResidualGain, 0.0);

        RelativeIntensityPatternParameters rigid = defaults.toBuilder()
                .fitRotation(true).maxRotationDegrees(7.5)
                .interpolation(Warper.Interpolation.BILINEAR).build();
        assertTrue(rigid.registrationOptions().aligner.fitRotation);
        assertEquals(Math.toRadians(7.5),
                rigid.registrationOptions().aligner.maxRotation, 1e-15);
        assertTrue(rigid.toBuilder().build().fitRotation);
        assertEquals(7.5, rigid.toBuilder().build().maxRotationDegrees, 0.0);
        assertTrue(rigid.toBuilder().build().incrementalRotation);
        assertEquals(0.005, rigid.toBuilder().build().minimumRotationResidualGain, 0.0);
    }

    @Test
    public void nearestNeighbourRigidOutputWarnsButIsNotChanged() {
        RelativeIntensityPatternParameters rigid = RelativeIntensityPatternParameters.builder().useRecommendation(false)
                .fitRotation(true).interpolation(Warper.Interpolation.NONE).build();
        assertEquals(Warper.Interpolation.NONE, rigid.interpolation);
        boolean warned = false;
        for (ripr.core.Registration.Warning warning
                : ripr.core.Registration.configurationWarnings(rigid.registrationOptions())) {
            warned |= warning.kind
                    == ripr.core.Registration.Warning.Kind.NEAREST_NEIGHBOUR_ROTATION;
        }
        assertTrue(warned);
        assertTrue(ripr.core.Registration.configurationWarnings(
                rigid.toBuilder().interpolation(Warper.Interpolation.BILINEAR)
                        .build().registrationOptions()).isEmpty());
    }
    ...
    @Test
    public void identicalSplitRecipesDelegateToTheValidatedSameRecipePathExactly() {
        ImagePlus input = rigidPair();
        RelativeIntensityPatternParameters recipe = RelativeIntensityPatternParameters.builder()
                .useRecommendation(false)
                .preprocessing(Preprocessing.GAUSSIAN_1_0)
                .reference(Reconciler.Reference.CONSECUTIVE)
                .autoMaxShift(false).maxShift(6).threads(1)
                .fitRotation(true).incrementalRotation(true)
                .minimumRotationResidualGain(0.0).build();
        try {
            ripr.core.Registration.Result direct = RelativeIntensityPatternRegistration.estimate(
                    input, recipe, null, null);
            ripr.core.Registration.Result split =
                    RelativeIntensityPatternRegistration.estimateWithRotationRecipe(
                            input, recipe, recipe, null, null);
            assertEquals(direct.pairs.size(), split.pairs.size());
            for (int t = 0; t < direct.cumulative.length; t++) {
                assertEquals(direct.cumulative[t].dx, split.cumulative[t].dx, 0.0);
                assertEquals(direct.cumulative[t].dy, split.cumulative[t].dy, 0.0);
                assertEquals(direct.cumulative[t].theta, split.cumulative[t].theta, 0.0);
                assertEquals(direct.residualAfter[t], split.residualAfter[t], 0.0);
            }
        } finally {
            input.close();
        }
    ...
    @Test
    public void publicSplitPathCanUseAreaTranslationAndMaskedLogRatioRotation() {
        ImagePlus input = rigidPair();
        RelativeIntensityPatternParameters translation = RelativeIntensityPatternParameters.builder()
                .useRecommendation(false)
                .estimator(PairEstimator.Kind.AREA_CORRELATION_NEWTON)
                .preprocessing(Preprocessing.GAUSSIAN_0_7)
                .reference(Reconciler.Reference.CONSECUTIVE)
                .autoMaxShift(false).maxShift(6).threads(1).build();
        RelativeIntensityPatternParameters rotation = translation.toBuilder()
                .estimator(PairEstimator.Kind.LOG_RATIO_FIT)
                .preprocessing(Preprocessing.GAUSSIAN_1_0)
                .pixelSelectionStrategy(PixelSelectionStrategy.REMOVE_LEAST_INFORMATIVE)
                .pixelRemovalPercent(25)
                .fitRotation(true).minimumRotationResidualGain(0.0).build();
        try {
            ripr.core.Registration.Result split =
                    RelativeIntensityPatternRegistration.estimateWithRotationRecipe(
                            input, translation, rotation, null, null);
            assertEquals(1, split.rotationAcceptedPairs());
            assertEquals(1.5, split.cumulative[1].thetaDegrees(), 0.25);
        } finally {
            input.close();
        }
    }
    ...
    @Test
    public void publicSplitPathCanGloballyProposeAngleThenRefitTranslation() {
        ImagePlus input = rigidPair();
        RelativeIntensityPatternParameters recipe = RelativeIntensityPatternParameters.builder()
                .useRecommendation(false)
                .reference(Reconciler.Reference.CONSECUTIVE)
        ...
        try {
            ripr.core.Registration.Result split =
                    RelativeIntensityPatternRegistration.estimateWithRotationRecipe(
                            input, recipe, recipe, true, null, null);
            assertEquals(1, split.rotationAcceptedPairs());
            assertEquals(1.5, split.cumulative[1].thetaDegrees(), 0.25);
        } finally {
            input.close();
    ...
    private static ImagePlus rigidPair() {
        // Odd dimensions guard the production pyramid's integer-halving support layout.
        int size = 65;
        float[] first = new float[size * size];
        java.util.Random random = new java.util.Random(6001L);
        for (int i = 0; i < first.length; i++) first[i] = 30 + 170 * random.nextFloat();
        float[] second = new float[first.length];
        Transform truth = new Transform(1.2, -0.8, Math.toRadians(1.5));
        Warper.warp(first, second, size, size, truth.inverse(),
                Warper.Interpolation.BILINEAR, Float.NaN);
        ImageStack stack = new ImageStack(size, size);
        stack.addSlice(new FloatProcessor(size, size, first));
        stack.addSlice(new FloatProcessor(size, size, second));
        return new ImagePlus("rigid pair", stack);
    }
}
