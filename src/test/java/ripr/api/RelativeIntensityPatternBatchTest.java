...
public class RelativeIntensityPatternBatchTest {
    ...
    public void discoveryIsSortedRecursiveAndExcludesNestedOutput() throws Exception {
        ...
        RelativeIntensityPatternBatchParameters parameters = RelativeIntensityPatternBatchParameters.builder(input, output,
                RelativeIntensityPatternParameters.builder().build()).recursive(true).build();
        List<File> found = RelativeIntensityPatternBatch.discover(parameters);
    ...
    public void sameInputAndOutputFolderStillDiscoversOriginals() throws Exception {
        ...
        RelativeIntensityPatternBatchParameters parameters = RelativeIntensityPatternBatchParameters.builder(input, input,
                RelativeIntensityPatternParameters.builder().build()).build();
        assertEquals(1, RelativeIntensityPatternBatch.discover(parameters).size());
    }
    ...
    public void processesStacksSequentiallyPreservesFoldersAndSkipsExistingOutputs() throws Exception {
        ...
        RelativeIntensityPatternBatchParameters parameters = RelativeIntensityPatternBatchParameters.builder(input, output, registration)
                .recursive(true).overwrite(false).build();

        RelativeIntensityPatternBatchResult first = RelativeIntensityPatternBatch.run(parameters);
        ...
        assertTrue(report.contains("rotation_recipe_id"));
        assertTrue(report.contains("rotation_proposal"));
        assertTrue(report.contains("rotation_selection_details"));
        assertTrue(report.contains("rotation_mode,rotation_events,rotation_event_window,"));
        assertTrue(report.contains("rotation_event_results"));
        assertTrue(report.contains("processed"));

        RelativeIntensityPatternBatchResult second = RelativeIntensityPatternBatch.run(parameters);
    ...
    public void badFileIsReportedWithoutStoppingTheRest() throws Exception {
        ...
        RelativeIntensityPatternParameters registration = RelativeIntensityPatternParameters.builder()
                ...
                .threads(1).crop(false).build();
        RelativeIntensityPatternBatchResult result = RelativeIntensityPatternBatch.run(RelativeIntensityPatternBatchParameters
    ...
    public void estimateUsesCompletedStackAverageAndCurrentFraction() {
        assertEquals(-1, RelativeIntensityPatternBatch.remaining(0, 0, 4, 0));
        assertEquals(3000, RelativeIntensityPatternBatch.remaining(2000, 2, 4, 1));
        assertEquals(3500, RelativeIntensityPatternBatch.remaining(2000, 2, 4, 0.5));
        assertEquals(0, RelativeIntensityPatternBatch.remaining(2000, 2, 0, 0));
    }
