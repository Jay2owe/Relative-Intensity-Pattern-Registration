...
public class PairEstimatorAxisTest {
    ...
    private static LogRatioParameters manual() {
    ...
    public void theEstimatorSurvivesAMacroRoundTrip() {
        for (PairEstimator.Kind kind : PairEstimator.Kind.values()) {
            LogRatioParameters original = manual().toBuilder().estimator(kind).build();
    ...
    public void theEstimatorSurvivesTheBatchParser() {
        LogRatioParameters parsed = MacroOptionsParser.parse(
    ...
    public void theRecipeCarriesTheEstimatorIntoTheParametersAndTheRunLog() {
        ...
        LogRatioParameters applied = recipe.applyTo(manual());
    ...
    private static final class ConstantSource implements logratio.core.FrameSource {
        @Override
        public int count() {
            return 2;
        }
