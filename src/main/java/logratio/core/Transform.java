 ...
 *
 * <pre>
 *   theta = theta1 + theta2                (exact â€” rotations about a common centre commute)
 *   t     = R(theta2) t1 + t2
 * </pre>
 *
 ...
 * ({@link #plus(Transform)}) is the first-order approximation in {@code theta}, correct to
 * within {@code |t1| * theta} â€” negligible for stage shake, which is a fraction of a degree,
 * and it is the only form in which the multi-lag least-squares reconciliation can work at
 * all, because that solves a linear system in the parameters. {@link Reconciler} documents
 * where each is used.
 *
