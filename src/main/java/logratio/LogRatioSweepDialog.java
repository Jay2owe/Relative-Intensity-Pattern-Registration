...
final class LogRatioSweepDialog {
    private LogRatioSweepDialog() { }

    static LogRatioParameters show(Window owner, ImagePlus image, LogRatioParameters base) {
        Configuration configuration = configure(image, base);
        if (configuration == null) return null;
        LogRatioSweepPlan plan = new LogRatioSweepPlan(base, configuration.axes);
        Results results = new Results(owner, image, plan, configuration.previewFrame);
        return results.showDialog();
    }

    private static Configuration configure(ImagePlus image, LogRatioParameters base) {
        ...
        LogRatioSweepPlan.Parameter[] defaults = {
                LogRatioSweepPlan.Parameter.PIXEL_SELECTION,
                LogRatioSweepPlan.Parameter.PIXEL_REMOVAL,
                LogRatioSweepPlan.Parameter.NONE
        };
        String[] labels = LogRatioSweepPlan.Parameter.labels();
        for (int i = 0; i < 3; i++) {
        ...
        dialog.addMessage("Maximum " + LogRatioSweepPlan.MAX_COMBINATIONS
                + " combinations. Runs are sequential to keep memory bounded.");
        dialog.setOKLabel("Run sweep");
        dialog.showDialog();
        if (dialog.wasCanceled()) return null;
        LogRatioSweepPlan.Axis[] axes = new LogRatioSweepPlan.Axis[3];
        for (int i = 0; i < axes.length; i++) {
            LogRatioSweepPlan.Parameter parameter = LogRatioSweepPlan.Parameter.fromLabel(dialog.getNextChoice());
            axes[i] = new LogRatioSweepPlan.Axis(parameter, dialog.getNextString());
        }
    ...
    private static String suggested(LogRatioSweepPlan.Parameter parameter, ImagePlus image) {
        if (parameter != LogRatioSweepPlan.Parameter.ESTIMATION_SCALE) return parameter.suggested;
    ...
    private static final class Configuration {
        final LogRatioSweepPlan.Axis[] axes;
        final int previewFrame;
        Configuration(LogRatioSweepPlan.Axis[] axes, int previewFrame) {
            this.axes = axes; this.previewFrame = previewFrame;
        }
    }

    private static final class Outcome {
        final LogRatioSweepPlan.Combination combination;
        ...
        Outcome(LogRatioSweepPlan.Combination combination, Registration.Result registration,
    ...
    private static final class Results {
        ...
        private final LogRatioSweepPlan plan;
        ...
        private LogRatioParameters accepted;
        private long start;

        Results(Window owner, ImagePlus image, LogRatioSweepPlan plan, int previewFrame) {
            this.image = image;
            this.plan = plan;
        ...
        private final class Runner extends SwingWorker<Void, Published> {
            @Override protected Void doInBackground() {
                for (int index = 0; index < plan.combinations.size(); index++) {
                    ...
                    LogRatioSweepPlan.Combination combination = plan.combinations.get(index);
                    long itemStart = System.currentTimeMillis();
                    try {
                        Registration.Result registration = LogRatioRegistration.estimate(image,
        ...
        private final class Card {
            ...
            Card(int index, LogRatioSweepPlan.Combination combination) {
    ...
    private static BufferedImage preview(ImagePlus source, LogRatioParameters parameters,
                                         Registration.Result result, int frame,
                                         int targetWidth, int targetHeight) {
        StackFrames frames = StackFrames.of(source, parameters.channel, parameters.slice);
        int referenceFrame = parameters.reference == logratio.core.Reconciler.Reference.FIXED
    ...
    static double fullResolutionResidual(ImagePlus source, LogRatioParameters parameters,
