...
public final class AutomaticInformationSelector {
    ...
    public static Result select(float[] first, float[] second, int width, int height,
        ...
        // The recipe selector consumes these values as evidence even when the older information
        // rule can decide from sparsity alone. Leaving them as NaN used to make downstream code
        // silently replace real sparse-recording evidence with a training mean.
        ...
        if (explicitIntensityBand) {
            return new Result(Choice.EXPLICIT_INTENSITY_BAND, sparse,
                    temporal.outlierFraction, temporal.tailRatio, true);
        }
        if (sparse < SPARSE_BOUNDARY) {
            return new Result(Choice.SPARSE_MUTUAL_NOISE_EDGES, sparse,
                    temporal.outlierFraction, temporal.tailRatio, false);
        }
