...
public final class StackWarper {
    ...
    public static final String PHASE_WARP = "Applying transforms to the output stack";
    ...
    public static ImagePlus apply(ImagePlus imp, Transform[] cumulative,
        ...
        return apply(imp, cumulative, interpolation, crop, PairScheduler.Progress.NONE,
                PairScheduler.Cancellation.NEVER);
    }

    /** Apply transforms with timepoint-level progress and cooperative cancellation. */
    public static ImagePlus apply(ImagePlus imp, Transform[] cumulative,
                                  Warper.Interpolation interpolation, boolean crop,
        ...
        prog.phase(PHASE_WARP, 0, frames);
        for (int t = 1; t <= frames; t++) {
            ...
            prog.phase(PHASE_WARP, t, frames);
        }
     ...
     *
     * <p>Values are clamped to the type's range on the way back. Bicubic and Fourier interpolation can
     * overshoot at a hard edge, so an unclamped 16-bit conversion would wrap a bright edge to black —
