...
// EXPERIMENTAL ONLY: double-precision sampling to reproduce frozen Round 17.
// Loaded ahead of the frozen Java jar by the Round 20 diagnostic harness.
 ...
 * than fitted â€” at every evaluation it is set to the median of the difference field, which is its L1
 ...
 * and in the reported residual. Mixing estimators â€” a weighted mean inside the iteration and a median
 * in the line search â€” makes the two costs different objectives, so the search can reject a good step
 ...
 *
 * <p>The solver. Forward-additive Gaussâ€“Newton with iteratively reweighted least squares, run coarse
 ...
 * coarsest level makes it affordable â€” a 30 px bound over four levels is a 9x9 integer sweep on a
 ...
 * Gaussâ€“Newton on image gradients, is the gain-invariant member of the Lucasâ€“Kanade family â€” the
...
public final class PairAligner {
    ...
    public enum PixelSupport {
         ...
         *
         * <p>A flat pixel carries no information about displacement at all â€” its Jacobian row is
         * zero â€” so excluding it removes work rather than information. On a mostly-empty field that
    ...
    public static final class Options {
        ...
        /** How a residual is penalised. See {@link RobustNorm} â€” this is the setting that matters. */
         ...
         * differences on log intensities â€” i.e. it removes the one property that distinguishes this
        ...
        /** Gaussâ€“Newton iterations per pyramid level. */
    ...
    public static final class Fit {
        ...
        /** Gaussâ€“Newton iterations used at the finest level. */
    ...
    public static Fit align(LogPlane[] a, LogPlane[] b, Options o) {
        ...
        // ---- descend, refining by Gaussâ€“Newton at each level --------------------------------
        int iterations = 0;
    ...
    private static Transform coarseSearch(LogPlane a, LogPlane b, int radius, double maxShiftHere,
        ...
        // a micro-optimisation: on a frame with no texture â€” blank, saturated, or all background â€”
        ...
        for (it = 0; it < o.maxIterations; it++) {
            ...
            for (int y = 0; y < a.height; y += s.stride) {
                ...
                for (int x = 0; x < a.width; x += s.stride) {
                    ...
                    double bv = b.sampleReference(out[0], out[1]);
                    if (Double.isNaN(bv)) continue;
                    double gxb = b.sampleGxReference(out[0], out[1]);
                    double gyb = b.sampleGyReference(out[0], out[1]);
                    if (Double.isNaN(gxb) || Double.isNaN(gyb)) continue;
            ...
            if (step == null) {
                // No gradient information at all: a featureless pair. Not a refusal â€” the transform
                // found so far is the best available â€” but not a convergence either.
                break;
            }
    ...
    private static final class Sample {
        ...
        final int n;
        /** Pixels that were visited â€” the denominator the validity check must use. */
        final int nPossible;
        final double gain;
    ...
    private static Sample evaluate(LogPlane a, LogPlane b, Transform p, double cx, double cy,
        ...
        for (int y = 0; y < a.height; y += s.stride) {
            ...
            for (int x = 0; x < a.width; x += s.stride) {
                ...
                if (Double.isNaN(bv)) continue;
                if (hardMutualNoise && targetNoiseThreshold > 0) {
                    double gx = b.sampleGxReference(out[0], out[1]);
                    double gy = b.sampleGyReference(out[0], out[1]);
                    if (Double.isNaN(gx) || Double.isNaN(gy)) continue;
