 ...
 * {@code g} the log image becomes {@code log I + log g} â€” a constant offset â€” so a cost built on the
 ...
 *
 * <p><b>Why the epsilon is not cosmetic.</b> A bare log diverges at zero, and zero is common â€”
 ...
 * is the log of an arithmetic mean. Only the former commutes with a global gain â€”
...
public final class LogPlane {
    ...
    /** No upper intensity limit â€” the default for {@link #of}. */
     ...
     *
     * <p><b>Why it lives on the plane rather than in a cache of its own.</b> The preparation â€”
     * undoing the log, centring on the plane mean, and solving the B-spline coefficients â€” is a pure
     ...
     * @param satMax    intensities at or above this are invalid â€” a saturated pixel has lost the
     ...
     *
     * <p>Each step is a 5-tap binomial blur (Burtâ€“Adelson {@code [1 4 6 4 1]/16}, sigma about 1.0)
     ...
     * asks for a search that dominates the whole run â€” a 500 px bound at one level is a million
     ...
     * uneven â€” a geometric mean is pulled towards the darkest pixel in the window, so a coarse level
     ...
     * {@code mean(I) + epsilon} â€” the offset passes through the average unchanged.
     */
     ...
     * gradient along the whole mask boundary â€” exactly the kind of frame-invariant edge that anchors
     * a registration at zero shift.
     */
    /** Python-reference reduction order; experimental overlay, not the production default. */
    private static Blurred blur5(float[] a, boolean[] ok, int w, int h) {
        float[] weighted = new float[a.length], weights = new float[a.length];
        for (int i = 0; i < a.length; i++) { weighted[i] = ok[i] ? a[i] : 0f; weights[i] = ok[i] ? 1f : 0f; }
        float[] horizontal = new float[a.length], share = new float[a.length];
        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++) {
            int index = y*w+x;
            float total = referenceKernelFive(weighted, y*w, x, w, 1);
            float count = referenceKernelFive(weights, y*w, x, w, 1);
            float value = count > 0 ? total/count : 0f;
            share[index] = count/16f;
            // The reference materialises this float32 product before the second convolution.
            horizontal[index] = value*share[index];
        }
        float[] output = new float[a.length]; boolean[] valid = new boolean[a.length];
        for (int y = 0; y < h; y++) for (int x = 0; x < w; x++) {
            int index=y*w+x;
            float total=referenceKernelFive(horizontal,x,y,h,w);
            float count=referenceKernelFive(share,x,y,h,w);
            output[index]=count>0?total/count:0f;
            valid[index]=count/16f >= VALID_WEIGHT_SHARE;
        }
        return new Blurred(output,valid);
    }

    private static float referenceKernelFive(float[] a,int base,int at,int length,int stride) {
        double sum=6.0*a[base+at*stride];
        sum+=a[base+clamp(at-2,length)*stride]+(double)a[base+clamp(at+2,length)*stride];
        sum+=4.0*(a[base+clamp(at-1,length)*stride]+(double)a[base+clamp(at+1,length)*stride]);
        return (float)sum;
    }
    ...
    public double sampleReference(double x, double y) {
        return sampleReferenceOf(v, x, y);
    }

    public double sampleGxReference(double x, double y) {
        return sampleReferenceOf(gx, x, y);
    }

    public double sampleGyReference(double x, double y) {
        return sampleReferenceOf(gy, x, y);
    }

    private float sampleOf(float[] a, double x, double y) {
        return (float) sampleReferenceOf(a, x, y);
    }

    private double sampleReferenceOf(float[] a, double x, double y) {
