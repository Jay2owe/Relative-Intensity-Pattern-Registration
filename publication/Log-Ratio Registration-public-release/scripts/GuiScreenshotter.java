...
public final class GuiScreenshotter {
    ...
    private static JDialog buildMain() {
        ...
        row = 0;
        message(body, "Choose the closest image and movement types, then choose where the");
        message(body, "settings come from. Automatic is the default: it inspects this recording and");
        message(body, "resolves pixel support, brightness limits, filter and mask, then shows you the");
        message(body, "result before it runs. Recommended loads the measured recipe for the chosen");
        message(body, "types and changes nothing. Manual uses exactly the values in the settings window.");
        ...
        choice(body, "Settings source", "Automatic full selection", "Recommended", "Manual");
        check(body, "Review and edit all settings before running", false);
        ...
        dialog.setPreferredSize(new Dimension(700, 520));
        return dialog;
    }

    private static JDialog buildAdvanced() {
        JDialog dialog = new JDialog((Frame) null, "Log-Ratio Registration - All Settings", false);
        JPanel body = new JPanel(new GridBagLayout());
        body.setBorder(new EmptyBorder(12, 14, 8, 14));
        row = 0;
        message(body, "Automatic full selection resolved this recipe for the current recording.");
        message(body, "All values below are ordinary editable settings.");
        section(body, "0. Pairwise estimator");
        message(body, "How one frame pair is measured. Sections 1 to 4 are settings of the log-ratio fit.");
        choice(body, "Estimator", "Log-ratio fit", "Area correlation (Newton)", "Area correlation (grid)");
        section(body, "1. Base pixel support");
        choice(body, "Pixels used to estimate movement", "ALL", "GRADIENT", "MUTUAL_NOISE_GRADIENT");
        field(body, "Gradient multiplier", "0.5");
        section(body, "2. Intensity restrictions");
        field(body, "Exclude dimmest pixels below percentile", "off");
        field(body, "Exclude brightest pixels above percentile", "off");
        section(body, "3. Estimation filter");
        choice(body, "Filter", "Median denoising (3 by 3)", "None", "Gaussian smoothing (0.7 pixel)",
                "Gaussian smoothing (1.0 pixel)", "Photon-noise stabilisation", "Mild sharpening");
        check(body, "Remove additive background", false);
        field(body, "Background percentile", "1.0");
        section(body, "4. Spatial pixel removal (optional second pass)");
        message(body, "A scoring copy chooses the mask; original unfiltered pixels drive the final fit.");
        choice(body, "Removal strategy", "Remove least spatially informative pixels",
        ...
        section(body, "5. Fit and performance");
        ...
        JPanel buttons = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        buttons.add(new JButton("Cancel"));
        JButton use = new JButton("Use these settings");
    ...
    private static JDialog buildBatch() {
        ...
        choice(body, "Motion type", "Subpixel random walk", "Curved / oscillating drift",
                "Steady directional drift", "Intermittent jumps");
        choice(body, "Settings source", "Automatic full selection", "Recommended", "Manual");
        message(body, "Automatic resolves a complete recipe separately for each stack and records it.");
        check(body, "Review and edit all settings before starting", false);
    ...
    private static JDialog buildSweepSetup() {
        ...
        choice(body, "Parameter 1", "Pixel removal strategy", "Pairwise estimator", "Preprocessing", "Estimation scale", "Robust weighting", "Pixel evidence",
