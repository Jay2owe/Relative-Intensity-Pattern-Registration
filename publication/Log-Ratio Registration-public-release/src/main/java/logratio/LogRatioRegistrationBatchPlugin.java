...
import logratio.api.LogRatioBatch;
import logratio.api.LogRatioBatchParameters;
import logratio.api.LogRatioBatchResult;
import logratio.api.LogRatioBatchStatus;
...
public final class LogRatioRegistrationBatchPlugin implements PlugIn {
    public static final String COMMAND = "Log-Ratio Registration Batch...";
    private static final String INPUT_PREF = "logratio.batch.input";
    private static final String OUTPUT_PREF = "logratio.batch.output";
    ...
    public void run(String arg) {
        ...
        try {
            LogRatioBatchParameters parameters;
            if (macro != null || headless) {
                parameters = LogRatioBatchMacroOptionsParser.parse(macro == null ? "" : macro);
            } else {
                parameters = dialog();
                if (parameters == null) return;
                if (Recorder.record) {
                    Recorder.recordString("run(\"" + COMMAND + "\", \""
                            + LogRatioBatchMacroOptionsParser.record(parameters) + "\");\n");
                }
            }
            int count = LogRatioBatch.discover(parameters).size();
            if (count == 0) throw new IllegalArgumentException("no TIFF stacks were found in the input folder");
            if (headless || macro != null) {
                finish(LogRatioBatch.run(parameters, LogRatioRegistrationBatchPlugin::logStatus,
                        logratio.core.PairScheduler.Cancellation.NEVER));
            } else {
                start(parameters, count);
            }
        } catch (RuntimeException error) {
            if (headless || macro != null) {
                IJ.log("Log-Ratio Registration Batch failed: " + rootMessage(error));
                throw error;
            }
            IJ.error("Log-Ratio Registration Batch", rootMessage(error));
        }
    }

    private static LogRatioBatchParameters dialog() {
        String home = System.getProperty("user.home", ".");
        GenericDialog dialog = new GenericDialog("Log-Ratio Registration Batch");
        ...
        dialog.addMessage("Automatic is the default. It resolves a complete recipe separately for\n"
                + "each stack and records the one it chose in the batch report. For the image\n"
                + "types it serves it registers each stack twice, so allow roughly double the\n"
                + "time; for the others it costs no more than Recommended. Manual replays one\n"
                + "explicit recipe over every stack.");
        ...
        if (advanced) {
            LogRatioParameters edited = LogRatioRegistrationPlugin.advancedDialog(registration);
        ...
        return LogRatioBatchParameters.builder(input, output, registration)
                .recursive(recursive).overwrite(overwrite).build();
    }

    private static void start(LogRatioBatchParameters parameters, int count) {
        AtomicBoolean cancel = new AtomicBoolean();
        BatchProgressWindow window = new BatchProgressWindow(count, cancel);
        window.showWindow();
        Thread worker = new Thread(() -> {
            try {
                LogRatioBatchResult result = LogRatioBatch.run(parameters, status -> {
    ...
    private static void logStatus(LogRatioBatchStatus status) {
        ...
        IJ.showStatus("Log-ratio batch: " + status.completedFiles + " / " + status.totalFiles
                + file + estimate);
    }

    private static void finish(LogRatioBatchResult result) {
        String state = result.cancelled ? "cancelled" : "complete";
        IJ.showProgress(result.cancelled ? 0 : 1.0);
        IJ.showStatus("Log-ratio batch " + state);
        IJ.log("Log-Ratio Registration Batch " + state + ": " + result.processedFiles
    ...
    private static final class BatchProgressWindow {
        ...
        private volatile LogRatioBatchStatus latest;
        private volatile long latestAt;
        private volatile boolean done;

        BatchProgressWindow(int total, AtomicBoolean cancellation) {
            this.cancellation = cancellation;
            dialog = new JDialog((Frame) null, "Log-Ratio Registration Batch", false);
        ...
        void finish(LogRatioBatchResult result) {
            SwingUtilities.invokeLater(() -> {
                done = true;
                timer.stop();
        ...
        private static String duration(long millis) {
            ...
            long minutes = (seconds % 3600) / 60;
            long remainder = seconds % 60;
            return LogRatioRegistrationBatchPlugin.duration(millis);
        }
    }
}
