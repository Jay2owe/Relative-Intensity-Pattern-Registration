...
final class LogRatioBatchMacroOptionsParser {
    private LogRatioBatchMacroOptionsParser() { }

    static LogRatioBatchParameters parse(String options) {
        ...
        LogRatioParameters registration = MacroOptionsParser.parseTokens(values);
        return LogRatioBatchParameters.builder(new File(input), new File(output), registration)
                .recursive(recursive).overwrite(overwrite).build();
    }

    static String record(LogRatioBatchParameters parameters) {
        return "input=[" + safePath(parameters.inputDirectory) + "] output=["
                ...
                + new LogRatioDialogModel(parameters.registration).toMacroOptions();
    }
