...
public final class PairAligner {
    ...
    public static Fit align(LogPlane[] a, LogPlane[] b, Options o) {
        ...
        for (int l = top; l >= 0; l--) {
            ...
            if (l > 0) p = clampShift(p.scaleTranslation(2.0), o.maxShift / (scale / 2));
        }
        ...
        if (status == Status.OK && p.magnitude() >= o.maxShift * (1 - 1e-9)) {
    ...
    static Fit report(LogPlane[] a, LogPlane[] b, Options o, Transform p, int iterations,
        ...
        if (status == Status.REFUSED_LOW_OVERLAP || validFrac < o.minValidFraction) {
            return new Fit(Transform.IDENTITY, zero.meanAbs, zero.meanAbs, 0.0,
                    validFrac, iterations, Status.REFUSED_LOW_OVERLAP);
        }
    ...
    public static Fit alignFrom(LogPlane[] a, LogPlane[] b, Options o, Transform start,
        ...
        int top = levels - 1;
        double topScale = 1 << top;
        Transform p = clampShift(start.scaleTranslation(1.0 / topScale), o.maxShift / topScale);
    ...
    private static Transform coarseSearch(LogPlane a, LogPlane b, int radius, double maxShiftHere,
        ...
        for (int dy = -radius; dy <= radius; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                ...
                // The radius is a rounded-up integer, so it can reach past the requested bound; a
                // sweep that ignored the bound could return a shift the user forbade, then have it
    ...
    private static Refined refine(LogPlane a, LogPlane b, Transform start,
        ...
        for (it = 0; it < o.maxIterations; it++) {
            ...
            for (int t = 0; t < 6; t++, f *= 0.5) {
                ...
                Transform trial = clampShift(raw, maxShift);
    ...
    /** Clamps translation to {@code maxShift}. Returns the argument unchanged when within bounds,
     *  which is how the caller detects that the bound was hit. */
